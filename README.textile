...
If the user is online, the plugin will transparently request new content from the remote server, firing the callback again if the content has changed. If the user is offline, the plugin will request the data from the remote server for the most recent request when the user comes back online. You can explicitly silence the callback if you wish if the user moves away from the part of the application where it would be useful without entering an area of the page using cached data.
...
<pre lang="ruby">
jQuery.retrieveJSON("/url", {data: "toSend"}, function(json, status, followUp) {
...
If the Ajax request was made after a successful hit from the cache, jQuery Offline will make the third parameter to the function **true**. You might use this third parameter to alert the user that a change is about to happen, if such a change would be particularly jarring. Note that the value of this third parameter may change in a future release to provide additional information; a future-proof check would be: <code>if( status == "success" && followUp )</code>.

Additionally, @jQuery.retrieveJSON@ will not make a request if a request is in-process for a particular URL/query-string combination.
...
<html manifest="application.manifest">
  <head>
    ...
    <script src="/javascripts/offline.jquery.js"></script>
...
* Remember the time that the original content was cached; then provide it to
  follow-up calls
